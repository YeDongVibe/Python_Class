...
num1 = int(input("첫번째 수를 입력하라"))
num2 = int(input("두번째 수를 입력하라"))

op = input("원하는 연산 입력하라(1 : sum / 2 : sub / 3 : mul / 4 : div)")

func = object_dic[op]
print('연산 프로그램')
print(func(num1,num2))

#다음과 같이 구성되는 문자열을 구분 문자(&, =)로 분리하여 딕셔너리로 반환하는 함수 작성 예: 문자열 'led=on&motor=off&switch=off'이고 구분 문자가 '&', '='일 때 {'led':'on', 'motor':'off', 'switch':off'} 반환.
#Hint: dict([['a','b'], ['c', 'd']]) → {'a': 'b', 'c': 'd'}
def st_parse(str):
    re = []
    pairs = str.split('&')
    for pair in pairs:
        re.append(pair.split('=')) # 쪼갠 순서대로 key-value쌍으로 묶어줌
    return dict(re)
